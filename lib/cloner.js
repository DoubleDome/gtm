...
function Cloner() {
    ...
    this.init = function(data) {
        ...
        grunt.log.writeln();
        async.eachSeries(this.templates, this.clone, function() {
            _this.finalize();
        });
    };
        ...
        git.exec('init', function(error, message) {
            ...
            git.exec('add', ['.'], function(error, message) {
                ...
                git.exec('commit', ['-m', '"initial commit..."'], function(error, message) {
                    if (_this.data.origin) {
                        grunt.log.write('remote... ');
                        git.exec('remote', ['add', 'origin', _this.data.origin], function(error, message) {
                        ...
                        grunt.log.error('\nSkipping push, missing remote git...');
                    }
                });
            });
        });
